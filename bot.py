...
session.headers["User-Agent"] = "KriptoBot-4H/2.0"
...
def analyze(symbol, btc_filter):
    ...
    return {
        ...
        "volume_percent": (volume_ratio - 1) * 100,
        "level": float(level),
        "funding_rate": funding_rate,
        "candle": current["close_time"].isoformat()
    }
...
def format_signal(signal):
    ...
    return (
        f"{emoji} {signal['direction']} ADAYI\n\n"

        f"🪙 Coin: {signal['symbol']}\n"
        f"💰 Fiyat: {signal['price']:.8g}\n\n"
...
def scan():
    ...
    if signals:
        ...
        logging.info(
            "Aday bulunamadı. Telegram mesajı gönderilmeyecek."
        )
